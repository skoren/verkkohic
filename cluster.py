...
sys.stderr.write("Lodaed match info with %d nodes and %d edges\n"%(matchGraph.number_of_nodes(), matchGraph.number_of_edges()))
translate.close()
...
for line in translate:
   ...
   if w == 0:
      hicGraph.add_edge(line[1], line[2], weight=1)
   else:
      w = w['weight'] + 1
      hicGraph[line[1]][line[2]]['weight'] = w
sys.stderr.write("Lodaed hic info with %d nodes and %d edges\n"%(hicGraph.number_of_nodes(), hicGraph.number_of_edges()))
...
for c in sorted(nx.connected_components(G), key=len, reverse=True):
   print("Connected component wiuth %d nodes is: %s"%(len(c), c))
   ...
   if C.number_of_nodes() > 1:
         ...
         if n in matchGraph:
            for ec in matchGraph.edges(n):
               ...
               if ec[0] in C and ec[1] in C:
                  C.add_edge(ec[0], ec[1], weight=-1*FIXED_WEIGHT)
      ...
      part=community.kernighan_lin.kernighan_lin_bisection(C, partition=[set(p1), set(p2)], max_iter=100000, weight='weight', seed=None)
